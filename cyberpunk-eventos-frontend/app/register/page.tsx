...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await registrar(data, tipoUsuario);

    if (!result.sucesso) {
      toast.error(result.erro || 'Erro ao criar conta');
    } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-black to-cyan-900 flex items-center justify-center p-4">
      <Card className="w-full max-w-md bg-black/50 border-cyan-400/30 backdrop-blur-sm p-8">
        <div className="flex items-center justify-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-cyan-400">Cadastro</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Input
              ...
              placeholder={userType === 'company' ? 'Nome da empresa' : 'Seu nome'}
              required
